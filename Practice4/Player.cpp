...
CTerrainPlayer::CTerrainPlayer(ID3D11Device *pd3dDevice)
{
	//비행기 메쉬를 생성한다.
	CCubeMesh *pCubeMesh = new CCubeMesh(pd3dDevice, 4.0f, 12.0f, 4.0f);
	SetMesh(pCubeMesh);
...
void CTerrainPlayer::Render(ID3D11DeviceContext *pd3dImmediateDeviceContext)
{
	DWORD nCurrentCameraMode = (m_pCamera) ? m_pCamera->GetMode() : 0x00;
	if ((nCurrentCameraMode == THIRD_PERSON_CAMERA) && m_pMesh)
	{
...
void CTerrainPlayer::ChangeCamera(ID3D11Device *pd3dDevice, DWORD nNewCameraMode, float fTimeElapsed)
{
	...
	case THIRD_PERSON_CAMERA:
		...
		SetGravity(D3DXVECTOR3(0.0f, -500.0f, 0.0f));
		SetMaxVelocityXZ(125.0f);
		SetMaxVelocityY(400.0f);
		m_pCamera = OnChangeCamera(pd3dDevice, THIRD_PERSON_CAMERA, nCurrentCameraMode);
		m_pCamera->SetTimeLag(0.25f);
...
void CTerrainPlayer::OnPlayerUpdated(float fTimeElapsed)
{
	...
	float fHeight = pTerrain->m_pHeightMap->GetHeight(d3dxvPlayerPosition.x, d3dxvPlayerPosition.z, bReverseQuad) + 6.0f;
...
void CTerrainPlayer::OnCameraUpdated(float fTimeElapsed)
{
	...
	float fHeight = pTerrain->m_pHeightMap->GetHeight(d3dxvCameraPosition.x, d3dxvCameraPosition.z, bReverseQuad) + 5.0f;
