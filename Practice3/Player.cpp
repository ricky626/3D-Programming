...
void CPlayer::Move(const D3DXVECTOR3& d3dxvShift, bool bUpdateVelocity)
{
	...
	else
	{
		...
		if (d3dxvPosition.x > 1900 || d3dxvPosition.x < -1900 ||
			d3dxvPosition.z > 860 || d3dxvPosition.z < -900)
		{
			return;
		}
...
void CPlayer::Update(float fTimeElapsed)
{
	...
	/*플레이어를 속도 벡터 만큼 이동한다. 속도 벡터에 fTimeElapsed를 곱하는 것은 
	속도를 시간에 비례하도록 적용한다는 의미이다.*/
