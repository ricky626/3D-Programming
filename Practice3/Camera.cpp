...
CCamera::CCamera(CCamera *pCamera)
{
	if (pCamera)
	{
		...
		m_pd3dcbViewProjection = pCamera->GetCameraConstantBuffer();
...
// 카메라 변환 행렬과 투영 변환 행렬을 쉐이더로 전달하기 위한 구조체에 대한 상수 버퍼를
// 갱신하고 디바이스 컨텍스트의 슬롯에 연결하는 함수이다.
...
CFirstPersonCamera::CFirstPersonCamera(CCamera *pCamera) : CCamera(pCamera)
{
	...
	if (pCamera)
	{
		...
		이것은 스페이스-쉽 카메라의 로컬 y-축 벡터가 
		어떤 방향이든지 1인칭 카메라(대부분 사람인 경우)의 로컬 y-축 벡터가 월드좌표의 y-축이 되도록 
